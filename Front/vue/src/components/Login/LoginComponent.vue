    ...
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" method="post" @submit.prevent="login">
...
function validateEmail(email) {
  if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) {
    ...
    error.value.email = 'This field must be email type'
  }
}
...
const error = ref({
      'email' : '',
      'password':'',
      'whole': store.loginError,
    }
);

watch(user.value,(currentValue) => {
  user.value = currentValue;
  validateEmail(currentValue.email);
})
</script>
